...
void StarClusterPattern::Apply(const bool &forceUpdateAllStars, const bool &onlyUpdateColors)
{
	...
	Jobs::Wait(scene->ForEach<StarInfo, StarClusterIndex, StarOrbit, StarOrbitOffset, StarOrbitProportion, SurfaceColor>(
		{},
		[&](int i,
			Entity entity,
			StarInfo &starInfo,
...
{
	const auto scene = GetScene();
	const bool check = reverse ? !m_useFront : m_useFront;
	const auto articleInfoList = check ? scene->GetOrSetPrivateComponent<Particles>(m_rendererFront.Get()).lock()->m_particleInfoList.Get<ParticleInfoList>()
	...
	Jobs::Wait(scene->ForEach<GlobalTransform, DisplayColor>(
		...
		[&](int i, Entity entity, GlobalTransform &globalTransform, DisplayColor &displayColor) {
			infos[i].m_instanceMatrix.m_value = globalTransform.m_value;
			infos[i].m_instanceColor = glm::vec4(displayColor.m_value * displayColor.m_intensity, 1.0f);
		},
		false)
	);
	articleInfoList->SetParticleInfos(infos);
}
...
void StarClusterSystem::PushStars(StarClusterPattern &pattern, const size_t &amount)
{
	const auto scene = GetScene();
	m_counter = 0;
	const auto stars = scene->CreateEntities(m_starArchetype, amount, "Star");
